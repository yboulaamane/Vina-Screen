...
Description:
    This script automates the docking of multiple ligands using Smina.
...
print("Description:")
print("    This script automates the docking of multiple ligands using Smina.")
...
print("Please enter the grid box coordinates and sizes for Smina.")
    ...
    with open(OUTPUT_CSV, 'w', newline='') as csvfile:
        ...
        for ligand in os.listdir(DOCKING_DIR):
            if ligand.endswith(".pdbqt") and '_docked' not in ligand:
                ...
                try:
                    smina_output = subprocess.check_output(command, stderr=subprocess.STDOUT).decode('utf-8')
                    ...
                    with open(CONSOLE_OUTPUT, 'a') as console_file:
                        ...
                        console_file.write(smina_output)
                        console_file.write("\n\n")

                    match = re.search(r'^\s*1\s+([-\d.]+)', smina_output, re.MULTILINE)
                    if match:
